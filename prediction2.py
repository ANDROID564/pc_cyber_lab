...
class activity:
    def activity(videofile):
                ...
                for i in range(len(x_dif)):
                    ...
                    if(abs(hori)<6 and abs(verti)>1):
                        ...
                        if(line1[0][1]>=ys[j]):
                            ...
                            for i in range(len(x_dif)):
                                if(x_dif[i]>2):
                                    count_crawl=count_crawl+1
                                    print (colored("person crawling","red"))
                                    Activity="person crawling"
                                    writer.writerow({'X_coordinates' : xs[i], 'Y_coordinates': ys[j],
                                                     'Distance':dist2[i],
                                                     'Direction_angle':angle_dif[i],'Activity':Activity,'id':4
                                                     })    
                        ...
                        for i in range(len(y_dif)):
                            if(y_dif[i]>2):
                                ...
                                #print(y_dif[i])
                                count_climbdown=count_climbdown+1
                                print(colored("person  going downward",'yellow'))
                                writer.writerow({'X_coordinates' : xs[i], 'Y_coordinates': ys[j],
                                 'Distance':dist2[i],
                                 'Direction_angle':angle_dif[i],'Activity':Activity,'id':8
                                 })
                        ...
                        '''
                    elif(abs(hori)>line0[0][1] and abs(hori)<line1[0][1]):     
                        count_jump=count_jump+1
                        print(colored("person jumping",'yellow'))
                        writer.writerow({'X_coordinates' : xs[i], 'Y_coordinates': ys[j],
                         'Distance':dist2[i],
                         'Direction_angle':angle_dif[i],'Activity':Activity,'id':9
                         })
        ...
        while(video.isOpened()):
            ...
            cv2.line(frame,line0[0],line0[1],(5,5,5),2)
            cv2.line(frame,line1[0],line1[1],(0,5,5),2)
            cv2.line(frame,line2[0],line2[1],(0,5,5),2)
            if ret:
        ...
        myFile = open('test_frames_main6f_array1.csv', 'w')  
        with myFile:    
            #writer = csv.writer(myFile)
            myFields = ['Activity'] 
            writer = csv.DictWriter(myFile, fieldnames=myFields, lineterminator='\n')    
            writer.writeheader()  #used for writing the header file
            writer.writerow({'Activity':predict})
